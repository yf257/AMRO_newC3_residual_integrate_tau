...
FindFermi::FindFermi( Ipp64f * param)
{
	...
	for (int i = 0; i < cdevs; i++)
	{
		for (int j = 0; j < fineN; j++)
		{
			...
			theta[i*fineN + j] = 2 * 3.1415926 / (fineN - 1) * j;
	...
	vdSin(nfinepoint, theta, argSin);
	vdCos(nfinepoint, theta, argCos);
...
int FindFermi::ReturnStart(Ipp64f * startpoint)
{
	...
	interfunc(theta, kx, ky, fineN, cdevs, gridN, temp1, rtemp, finthe);
	...
	for (int i = 0; i < cdevs; i++)
	{
		for (int j = 0; j < gridN; j++)
		{
			...
			theta[i*gridN + j] = finthe[i*gridN + j];
